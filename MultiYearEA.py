...
if __name__ == "__main__":
    ...
    log_path = os.path.join(base_path, "MultiYear_EA_Logs.txt")
    ...
    problem = EA_Problem.Field_Optimization_Multi_Year(nut)

    # instantiate the optimization algorithm
    algorithm = personal_NSGAII(problem, population_size=100, variator=Crossover.row_cross(2, years=3))

    Logger.log(logging.INFO, "Starting at: " + str(datetime.datetime.now().time()))
    # optimize the problem using 10,000 function evaluations
    algorithm.log_frequency = 250
    algorithm.run(1000, base_path)
    ...
    with open(os.path.join(base_path, "Multi_Year-EA-Result.json"), "w") as f:

...
class Field_Optimization_Multi_Year(Problem):

    def __init__(self, Nutrition):
        # 3 Year-setup 0:75 = Plants | 75: = Fertilizer
        super().__init__(84, 3)
        self.types[0:75] = Integer(0, 3)
        self.types[75:] = Real(0, 2)
        self.directions[1:] = Problem.MINIMIZE
        self.directions[0] = Problem.MAXIMIZE
        self.nutrition = Nutrition
        GlobalConfig.log_nutrition = False
    ...
    def evaluate(self, solution):
        ...
        fertilizer_list = solution.variables[75:78]
        fertilizer = NutritionTable.NutritionTable(fertilizer_list[0], fertilizer_list[1], fertilizer_list[2])
        g.fertilizer = fertilizer
        ...
        for x in range(5):
            for y in range(5):
                plants[x][y] = plant_list[0][(plants_index[x * 5 + y], 0)]
        g.set_plants(plants)
        g.set_nutrition(grid_nutrition)
        # simulate Year 1
        ...
        cost = HelperFunctions.nutrition_to_cost(g.fertilizer) * 25 * GlobalConfig.max_steps
        #setup Year 2
        fertilizer_list = solution.variables[78:81]
        fertilizer = NutritionTable.NutritionTable(fertilizer_list[0], fertilizer_list[1], fertilizer_list[2])
        g.fertilizer = fertilizer
        plants_index = solution.variables[25:50]
        plants = [[plant_list[0][(0, 0)] for y in range(5)] for x in range(5)]
        for x in range(5):
            for y in range(5):
                plants[x][y] = plant_list[0][(plants_index[x * 5 + y], 0)]
        ...
        cost += HelperFunctions.nutrition_to_cost(g.fertilizer) * 25 * GlobalConfig.max_steps
        # setup Year 3
        fertilizer_list = solution.variables[81:]
        fertilizer = NutritionTable.NutritionTable(fertilizer_list[0], fertilizer_list[1], fertilizer_list[2])
        g.fertilizer = fertilizer
        plants_index = solution.variables[50:75]
        plants = [[plant_list[0][(0, 0)] for y in range(5)] for x in range(5)]
        for x in range(5):
            for y in range(5):
                plants[x][y] = plant_list[0][(plants_index[x * 5 + y], 0)]
